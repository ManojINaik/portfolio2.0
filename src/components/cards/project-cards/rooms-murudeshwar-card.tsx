import { Badge } from "../../shadcn/Badge";
import CardWrapper from "../card-wrapper";
import PullRequestIcon from "@/components/lottie-ui/pull-request-icon";

const RoomsMurudeshwarCard = () => {
  return (
    <CardWrapper>
      <div className="relative h-full flex flex-col justify-between gap-2 py-3 sm:py-4 px-4 sm:px-6 overflow-hidden">
        <div className="relative flex-1 flex items-center justify-center">
          <div className="relative h-[85%] w-[90%] rounded-lg overflow-hidden shadow-lg">
            <video
              src="/videos/roomsmurudeshwar.mp4"
              className="h-full w-full object-cover"
              muted
              autoPlay
              loop
              playsInline
            />
          </div>
        </div>
        <div className="flex flex-col items-start gap-1">
          <h4 className="text-xs font-medium opacity-80 uppercase flex flex-wrap gap-2 items-center">
            <span>Project</span>
          </h4>
          <div className="w-full flex gap-2 items-center justify-between">
            <h1 className="text-2xl font-medium dark:text-white text-black uppercase opacity-90">
              Rooms Murudeshwar
            </h1>
            <PullRequestIcon link="https://rooms.murudeshwar.co.in" />
          </div>
          <div className="flex gap-2 flex-wrap mt-1">
            <Badge variant="default" className="text-xs">HTML5</Badge>
            <Badge variant="default" className="text-xs">CSS3</Badge>
            <Badge variant="default" className="text-xs">JavaScript (ES6+)</Badge>
            <Badge variant="default" className="text-xs">jQuery</Badge>
            <Badge variant="default" className="text-xs">Bootstrap</Badge>
            <Badge variant="default" className="text-xs">Apache</Badge>
            <Badge variant="default" className="text-xs">Performance</Badge>
            <Badge variant="default" className="text-xs">SEO</Badge>
          </div>
        </div>
      </div>
    </CardWrapper>
  )
}

export default RoomsMurudeshwarCard
